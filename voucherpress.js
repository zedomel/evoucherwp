...
jQuery(document).ready( function( $ ) {
	jQuery('.voucher-delete').on('click', function(e) {
		var voucherId = jQuery(this).attr('name');
		jQuery.ajax({
			type: 'POST',
			dataType: 'json',
			url: url,
			...
			success: function( data ){
				if ( data.status ){
					...
					jQuery('.voucher-message').children().remove();
					jQuery('.voucher-message').prepend('<div class="error">' + data.message + '</div>');
				}
			}
		});
	});

	jQuery('.live-cb').on('change', function(e){

		// var items = [];
		// jQuery('table tbody tr').each( function (index, item){
		// 	items.push({
		// 		id: item.id,
		// 		live: jQuery(item).find('#live').is(':checked')
		// 	}); 
		// });
		// if (items.length > 0){
			jQuery.ajax({
				...
				data: {
					'action'	: 'save_voucher',
					'id'		: jQuery(this).parent().parent().attr('id'), //JSON.stringify(items),
					'live'		: jQuery(this).is(':checked')
				},
				success: function( data ){
					jQuery('.voucher-message').children().remove();
					if ( data.status ){
						jQuery('.voucher-message').prepend('<div class="updated">Successful save changes</div>');
					}
					else{
						jQuery('.voucher-message').prepend('<div class="error">Error saving changes</div>');
					}
				}
			});
		// }
