...
function voucherpress_init() {
    if ( function_exists( "add_action" ) ) {
        ...
        add_action( 'admin_enqueue_scripts', 'voucherpress_enqueue_scripts' );

        add_action( 'wp_ajax_delete_voucher', 'delete_voucher' );

        add_action( 'wp_ajax_save_voucher', 'save_voucher' );
...
function voucherpress_enqueue_scripts(){

    wp_register_script( 'voucherpress-script', plugin_dir_url( __FILE__ ) . '/voucherpress.js' );
    wp_localize_script( 'voucherpress-script', 'vp_siteurl', get_option( 'siteurl' ) );
    wp_localize_script( 'voucherpress-script', 'url', admin_url( 'admin-ajax.php' ) );
    wp_enqueue_script( 'voucherpress-script' );

    wp_enqueue_style( 'voucherpress-style', plugin_dir_url( __FILE__ ) . '/voucherpress.css' );
}

function delete_voucher(){
    $id = $_POST['id'];
    if ( !empty( $id ) ){
        voucherpress_delete_voucher( $id );
        echo json_encode( array( 'status' => true, 'id' => $id ) );
        die();
    }
    echo json_encode( array( 'status' => false, 'id' => $id,  'message' => __("Can't delete voucher", 'voucherpress') ) );
    die();
}
...
function save_voucher(){
    ...
    if ( !empty( $id ) && !empty( $live ) ){
        ...
        set live = %d
        where id = %d and blog_id = %d;", ($live === 'true' ? 1 : 0), $id, $blog_id );
        $ret = $wpdb->query( $sql );

        if ( false ===  $ret){
            echo json_encode( array( 'status' =>  false, 'message' => "Can't save changes") );
            die();
        }
        else{
            echo json_encode( array( 'status' => true, 'message' => "Successfully save changes") );
            die();
        }
    }
    die();
...
function vouchers_list() {
    ...
    if ( $vouchers && is_array( $vouchers ) && 0 < count( $vouchers ) ) {
        ...
        echo '<div class="voucher-message"></div>';
        voucherpress_table_header( array( "Customer e-mail", "Order", "Product", "Voucher Code", "Voucher Number", "Name", "Live", "Expiry", "Action" ) );
        foreach ( $vouchers as $voucher ) {
            $live = $voucher->live ? 'checked' : '';
            $order = wc_get_order( $voucher->order_id ); 
            $product_link = '#';
            $product_title = '';
            if ( !empty( $order ) ){
                $item['item_meta'] = $order->get_item_meta( $voucher->order_item_id );
            ...
            echo '
                    <tr id="' . $voucher->id . '">    
                            <td>' . $voucher->user_email . '</td>
                            <td><a href="' .  get_edit_post_link( $voucher->order_id ) . '">#' . $voucher->order_id . '</a></td>
                            <td><a href="' .  esc_url( $product_link ) . '">#' . $product_title . '</a></td>
                            <td>' . $voucher->code . '</td>
                            <td>' . $voucher->guid . '</td>
                            <td><a href="admin.php?page=vouchers&amp;id=' . $voucher->id . '">' . $voucher->name . '</a></td>
							<td><input type="checkbox" class="live-cb" name="live" id="live" ' . $live . '></td>
							<td>' . voucherpress_date( $voucher->expiry ) . '</td>
                            <td><a class="voucher-delete" name="' . $voucher->id . '" href="#">Delete</a></td> 
                    </tr>
                    ';
        }
        voucherpress_table_footer();
        echo '<a href="#" id="save-button" class="button button-primary">Save</a>';
...
function voucherpress_create_voucher_page() {
    ...
    <p><label for="ordercodes">' . __( "Use order number codes", "voucherpress" ) . '</label>
    <input type="radio" name="codestype" id="ordercodes" value="order" /> <span>' . __( "Tick this box to use order number codes on each voucher", "voucherpress" ) . '</span></p>
...
function voucherpress_edit_voucher_page() {
	...
	<input type="text" class="num" name="expirydays" id="expirydays" value="" /></p>
		...
		<p>' . __( "The code prefix and suffix will only be used on random, sequential and order codes.", "voucherpress" ) . '</p>
	...
	<input type="text" name="codesuffix" id="codesuffix" value="' . $voucher->codesuffix . '" /> <span>' . __( "Text to show after the sequential code (eg ABC123<strong>XYZ</strong>)", "voucherpress" ) . '</span></p>
        ...
        <p><label for="ordercodes">' . __( "Use order codes", "voucherpress" ) . '</label>
        <input type="radio" name="codestype" id="ordercodes" value="order"';
        if ( "order" == $voucher->codestype ) {
            echo ' checked="checked"';
        }
        echo ' /> <span>' . __( "Tick this box to use order number codes on each voucher", "voucherpress" ) . '</span></p>
...
function voucherpress_check_create_voucher() {
    if ( wp_verify_nonce( @$_POST["_wpnonce"], 'voucherpress_create' ) && "vouchers-create" == @$_GET["page"] && "" == @$_GET["preview"] && @$_POST && is_array( $_POST ) && 0 < count( $_POST ) ) {
        ...
        if ( "random" == $_POST["codestype"] || "sequential" == $_POST["codestype"] || "custom" == $_POST["codestype"] || "single" == $_POST["codestype"] || "order" == $_POST["codestype"] ) {
...
function voucherpress_create_voucher( $name, $require_email, $limit, $text, $description, $template, $font, $terms, $startdate, $expiry, $codestype, $codelength, $codeprefix, $codesuffix, $codes, $email = '', $order_id = '', $order_item_id = '' ) {
    ...
    if ( !empty( $email ) && !empty( $order_id )  && !empty( $order_item_id ) ){
        $code = '';
        if ( $codestype == 'order')
            $code = voucherpress_order_code( $order_id, $order_item_id );

        $sql = $wpdb->prepare( "insert into " . $prefix . "voucherpress_vouchers 
    (blog_id, name, `text`, `description`, terms, template, font, require_email, `limit`, guid, time, live, startdate, expiry, codestype, codelength, codeprefix, codesuffix, codes, deleted, user_email, order_id, order_item_id, code) 
    values 
    (%d, %s, %s, %s, %s, %d, %s, %d, %d, %s, %d, %d, %d, %d, %s, %d, %s, %s, %s, 0, %s, %d, %d, %s);", $blog_id, $name, $text, $description, $terms, $template, $font, $require_email, $limit, $guid, time(), 1, $startdate, $expiry, $codestype, $codelength, $codeprefix, $codesuffix, $codes, $email, $order_id, $order_item_id, $code );    
    }
    else {
        $sql = $wpdb->prepare( "insert into " . $prefix . "voucherpress_vouchers 
...
function voucherpress_get_vouchers( $num = 25, $all = false, $start = 0 ) {
    ...
    $sql = $wpdb->prepare( "select v.id, v.name, v.user_email, v.order_id, v.order_item_id, v.code, v.`text`, v.`description`, v.terms, v.require_email, v.`limit`, v.live, v.startdate, v.expiry, v.guid, 
(select count(d.id) from " . $prefix . "voucherpress_downloads d where d.voucherid = v.id and d.downloaded > 0) as downloads
from " . $prefix . "voucherpress_vouchers v
...
function voucherpress_create_code( $voucherid ) {
    ...
    if ( "custom" == $voucher_codestype->codestype ) {
        ...
        // using order codes
    } else if ( "order" == $voucher_codestype->codestype && !empty( $voucher_codestype->code ) ){
        // create code using order_id+product_id+random_integer
        $code = $voucher_codestype->codeprefix . $voucher_codestype->code . $voucher_codestype->codesuffix;
    }
    else {
...
function voucherpress_order_code( $order_id, $order_item_id ){
    $r = mt_rand(0, 99);
    return sprintf('%06d-%010d-%02d', intval($order_id) , intval( $order_item_id ), $r);
}
